...
**Allocate_pairs_PDB(struct prot_class **prot_class,
		     ...
		     int N_ij, int  N_U, int *N_L, int N_C,
		     int **L_bin, int L_short, int L_long)
{
  ...
  *L_bin=malloc((*N_L)*sizeof(int));
  //int L=L_min;
  //for(int i_L=0; i_L<(*N_L); i_L++){L+=L_STEP; (*L_bin)[i_L]=L;}
  (*L_bin)[0]=L_short;
  (*L_bin)[1]=L_long; 

  //Na2=Naa*Naa;
  int Np_C=N_C*(N_C+1)/2;
  int Np=N_U*(*N_L), Npair=3*Np_C*N_ij;
	      ...
	      int **label_ij)
{
  ...
  int k=0, na=-1, *nc=protein->n_cont; short *i_aa=protein->i_aa;
  ...
  for(int i=0; i<L_seq; i++){
	...
	printf("ERROR in set_pair\n"); exit(8);
      ...
      pair->ij+=ij;
      int nc1, nc2, i_c, j_c;
      if(nc[i]<nc[j]){nc1=nc[i]; nc2=nc[j];}
      else{nc1=nc[j]; nc2=nc[i];}
      pair->nc1+=nc1; pair->nc2+=nc2;
      if(nc1<C_low){i_c=0;}else if(nc1>C_hig){i_c=2;}else{i_c=1;}
      if(nc2<C_low){j_c=0;}else if(nc2>C_hig){j_c=2;}else{j_c=1;}
      if(pair->i_c!=i_c || pair->j_c!=j_c){
	printf("ERROR, class %d diff num of contacts %d %d not %d %d\n",
	       k, i_c, j_c, pair->i_c, pair->j_c);
	printf("C_nat= %d %d %d ij= %d\n",
	       pair->C_nat, C_mat[i][j], C2[i][j], ij);
...
void Compute_Phi(struct pair_class *pair, struct prot_class *prot)
{
  ...
  int error=0;
  if(isnan(pair->Phi0)){printf("ERROR, Phi0 is nan\n"); error++;}
  if(isnan(pair->Phi11)){printf("ERROR, Phi11 is nan\n"); error++;}
  if(isnan(pair->Phi01)){printf("ERROR, Phi01 is nan\n"); error++;}
  if(isnan(pair->Phi2)){printf("ERROR, Phi2 is nan\n"); error++;}
  if(error){
    printf("C_ij= %.3f\n", pair->Cont_freq);
    printf("C_ij_Nc= %.3f\n", pair->Cont_freq_Nc);
    printf("C_ij_nc= %.3f\n", pair->Cont_freq_nc);
...
float Normalize_Qprime(float *Q, struct pair_class  *pair)
{
  ...
  q=Q; if(sum){for(a=0; a<Na2; a++){(*q)/=sum; q++;}}
		  ...
		  int nc1, int nc2)
{
  ...
  if(nc1<C_low){i_c=0;}else if(nc1>C_hig){i_c=2;}else{i_c=1;}
  if(nc2<C_low){j_c=0;}else if(nc2>C_hig){j_c=2;}else{j_c=1;}
  if(j_c<i_c){int tmp=i_c; i_c=j_c; j_c=tmp;}
  if(i_c==0){ncp=j_c;}
  else if(i_c==1){ncp=2+j_c;}
  else{ncp=5;}
  if(C_mat){j=1;}
  else if(C2){j=2;}
  else{j=0;}
  int n=j*6*N_ij+ncp*N_ij+i;
  return(n);
}
		  ...
		  double *N2_obs, double n)
{
  // n=sum_c N2[c]
  int a, b, ab=0;
  for(int c=0; c<Na2; c++){Q_obs[c]=1.0;}
  for(a=0; a<Naa; a++){
    float na=P1_i[a]*n;
    for(b=0; b<Naa; b++){
	    ...
	    int LS, int comp, float *f_indir)
{
  ...
  // Predict Q
  double norm=0; int noconv=0, sum=0; printconv=0;
  for(int j=0; j<Nij_class; j++){
    ...
    for(int contact=0; contact<=1; contact++){
      ...
      for(int c=0; c<Na2; c++){
	if(isnan(Q[c])){
	  printf("ERROR contact %d %d, Q is nan\n", contact, c);
	  exit(8);
	}
      }
      ...
      int error=0;
      for(int c=0; c<Na2; c++){
	if(isnan(pair->prot_class->Q_global[c])){
	  if(error==0){printf("ERROR, Q_global is nan ");}
	  error++;
	}
      }
      if(error){printf("%d times\n", error); exit(8);}
      ...
      if(isnan(pair->score.log_lik[contact])){
	printf("ERROR, log.lik is nan\n");
      }
		  ...
		  int LS, int comp, float f_indir)
{
  ...
  double Score=0; float Qtmp[Na2], *Q, Q2[Na2], e;
  for(int j=0; j<Nij_class; j++){
    struct pair_class *pair=pair_p+j;
    if(pair->n==0)continue;
	...
	// Update likelihood
	if(contact==0){
	  pair->score.log_lik[2]=Likelihood_2(Qtmp, pair);
	  Copy_vec(Q2, Qtmp, Na2);
	}else{
	  float ll=Likelihood_2(Qtmp, pair);
	  if(ll>pair->score.log_lik[2]){pair->score.log_lik[2]=ll;}
	  Copy_vec(Q2, Qtmp, Na2);
	}
      } 
	...
	Copy_vec(pair->Q_pred_all, Q, Na2);
      ...
      if(contact==1 && (pair->score.log_lik[2] > pair->score.log_lik_opt)){
	if(f_indir)pair->error=0;
	pair->score.c_pred=2;
	pair->score.log_lik_opt=pair->score.log_lik[2];
	Copy_vec(pair->Q_pred_all, Q2, Na2);
      }
    } // end contact
		     ...
		     float f_indir)
{
  ...
  for(ik=0; ik<Nij_class; ik++){
    ...
    if(l_ik<pair_ij->prot_class->L){
      jk=Find_ij_class(l_ik, 1, 0, N_ij, ij_bin,
		       pair_ik->nc1, pair_ik->nc2); // contact
      Indirect_Q_k(Pk, Naa, pair_ij, pairs+ik, pairs+jk);
      w=(pairs[ik].n*pairs[jk].n); norm+=w; //w*=L;
      for(c=0; c<Na2; c++)P_ind[c]+=w*Pk[c];

      jk=Find_ij_class(l_ik, 0, 0, N_ij, ij_bin,
		       pair_ik->nc1, pair_ik->nc2); // no contact
      Indirect_Q_k(Pk, Naa, pair_ij, pairs+ik, pairs+jk);
      w=(pairs[ik].n*pairs[jk].n); norm+=w; //w*=L;
      for(c=0; c<Na2; c++)P_ind[c]+=w*Pk[c];
    }
...
float Likelihood_2(float *Q_pred, struct pair_class *pair)
{
  ...
  for(int c=0; c<Na2; c++){
    if(Q_pred[c]>qmin){l=log(Q_pred[c]);} 
    else{l=lmin;}
		  ...
		  int Nij_class, float Lambda, float f_indir, int L_tar)
{
  ...
  for(int j=0; j<Nij_class; j++){
    ...
    pair->score.r=
      ...
      Corr_coeff(&(pair->score.offset), &(pair->score.slope),
		   pair->Q_pred_all, pair->Q_obs, Na2);
...
void Pairwise_statistics(struct pair_class *pairs, int Npairs)
{
  ...
  for(int i=0; i<Npairs; i++){
    ...
    if(pair->n==0){empty[pair->C_nat]++; continue;}
    Symmetrize_mat(pair->N2_obs);
    double n=Marginalize_N2(pair->P1i_obs, pair->P1j_obs, pair->N2_obs);
    if(n==0){continue;}
  ...
  for(c=0; c<3; c++){
    empty_all+=empty[c];
    pairs->prot_class->Npair[c]=np-empty[c];
  } 
  pairs->prot_class->pairs=(Npairs-empty_all);
		      ...
		      struct pair_class *pairs, int Npairs)
{
  ...
  if(n<=0){
    for(int c=0; c<Na2; c++){
      prot->Q_global[c]=1; prot->log_Q_glob[c]=0;
    }
    return;
  }
  Pairwise_Q(prot->Q_global, prot->P1_i, prot->P1_j, N2_global, n);
  printf("Q_global computed\n");
  // Q_global(a,b)= N2(a,b)/P1(a)P1(b)
  int error=0;
  for(int c=0; c<Na2; c++){
    if(isnan(prot->Q_global[c])){
      if(error==0){printf("ERROR, Q_global is nan ");}
      error++;
    }
    if(prot->Q_global[c]){prot->log_Q_glob[c]=log(prot->Q_global[c]);}
    else{prot->log_Q_glob[c]=0;}
  }
  if(error){printf("%d times\n", error); exit(8);}
			  ...
			  int iprot, int all_prot)
{
  ...
  int error=0;
  for(int c=0; c<Na2; c++){
    if(isnan(prot_res->Q_global[c])){
      if(error==0){printf("ERROR in Compute_Q_global_all, Q_global is nan ");}
      error++;
    }
  }
  if(error){printf("%d times\n", error); exit(8);}
			 ...
			 int iprot, int all_prot)
{
  ...
  double Qnew_global[Na2], norm=0; int a;
  ...
  struct prot_class *prot_res=prots;
  if(all_prot==0)prot_res=prots+iprot;
...
void APC_pairs(struct pair_class *pairs, int Npairs, int L, char *what)
{
  ...
  for(i=0; i<Npairs; i++){
    pair=pairs+i;
    float APC=0;
    if(pair->i>=0 && pair->i<L && pair->j>=0 && pair->j<L){
      APC=average[pair->i]*average[pair->j];
    }
